...
function switchTab(name, btn) {
  ...
  if (name === 'lista')     cargarClientes();
  if (name === 'tipos')     cargarTipos();
  if (name === 'registrar') cargarSelectTipos();
}
...
async function cargarClientes() {
  try {
      ...
      <table id="tbl-cli">
        <thead>
          <tr>
            ...
            <th>Celular</th><th>Email</th><th>Registro</th>
          </tr>
        </thead>
        <tbody>
          ${data.map(c => `<tr>
    ...
    document.getElementById('cont-clientes').innerHTML = '<p style="color:red">Error al cargar</p>';
  }
}
...
async function cargarTipos() {
  try {
    ...
    document.getElementById('cont-tipos').innerHTML = `
      <table>
        <thead><tr><th>#</th><th>Nombre</th><th>Descripcion</th></tr></thead>
        <tbody>
          ${data.map(t => `<tr>
    ...
    document.getElementById('cont-tipos').innerHTML = '<p style="color:red">Error al cargar</p>';
  }
}
...
async function cargarSelectTipos() {
  try {
    ...
    sel.innerHTML = '<option value="">-- Seleccionar --</option>' +
      data.map(t => `<option value="${t.idTipoCliente}">${t.nombreTipoCliente}</option>`).join('');
  } catch (e) {}
}
...
async function registrarCliente() {
  ...
  try {
    ...
    if (!res.ok) return msg('msg-cliente', 'Error: ' + data.error, 'err');
    msg('msg-cliente', 'Cliente registrado con ID: ' + data.idCliente, 'ok');
    ['cli-nombre', 'cli-doc', 'cli-cel', 'cli-email', 'cli-fecha', 'cli-dir']
      .forEach(id => document.getElementById(id).value = '');
    document.getElementById('cli-tipo').value = '';
  } catch (e) {
    msg('msg-cliente', 'Error de conexion', 'err');
  }
}
...
async function registrarTipo() {
  ...
  try {
    ...
    const data = await res.json();
    if (!res.ok) return msg('msg-tipo', 'Error: ' + data.error, 'err');
    ...
    msg('msg-tipo', 'Error de conexion', 'err');
  }
}
